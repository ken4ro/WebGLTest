...
public class VersionUpdateManager : SingletonBase<VersionUpdateManager>
{
    ...
    public bool IsClientUpdate()
    {
        ...
        // 最新のクライアントバージョン取得
        var latestVersions = LoadServerVersionsSync();
    ...
    /// アセット更新(同期)
    /// </summary>
    public void AssetUpdateSync()
    {
        var serverVersions = LoadServerVersionsSync();
        ...
        if (IsAvatarUpdate(avatarVersion))
        {
            ...
            UpdateAvatarSync(avatarVersion);
        }
        else
        {
            // 更新不要
        ...
        if (IsResourcePackUpdate(resourcePackVersion))
        {
            ...
            UpdateResourcePackSync(resourcePackVersion);
        }
        else
        {
            // 更新不要
    ...
    // サーバーのバージョン情報をロード(同期)
    private ResponseAssetBundleVersions LoadServerVersionsSync() => WebServerManager.Instance.RequestFileVersionsSync();
    ...
    */
    ...
    // アバター更新(同期)
    private void UpdateAvatarSync(ResponseAssetBundleVersion avatarVersion)
    {
        // ファイルダウンロード
        var bytes = WebServerManager.Instance.RequestFileSync(avatarVersion.FileId);
    ...
    // リソースパック更新(同期)
    private void UpdateResourcePackSync(ResponseAssetBundleVersion resoucePackVersion)
    {
        // ファイルダウンロード
        var bytes = WebServerManager.Instance.RequestFileSync(resoucePackVersion.FileId);
