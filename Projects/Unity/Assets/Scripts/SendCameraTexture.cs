...
{
    [SerializeField]
    //NvPipeEncodeManager nvPipeEncodeManager = null;
    ...
    void Start()
    {
        ...
        Debug.Log($"VideoCodecType is {GlobalState.Instance.ApplicationGlobalSettings.VideoCodec}.");
    }
...
#if true
                            Debug.LogError("WebGL版はunitywebp非対応です");
#else
                            // WebPにエンコード
                            _tmpTexture2D.LoadRawTextureData(buffer);
                            _tmpTexture2D.Apply();
...
#if true
                            Debug.LogError("WebGL版はH264非対応です");
#else
                            // テクスチャセット
                            // H.264エンコードは非同期で行われる
